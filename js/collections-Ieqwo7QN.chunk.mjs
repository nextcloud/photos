import{a as $}from"./dav-Vt8ZXmmv.chunk.mjs";import{l as h,t as s,d as P}from"./index-BryCpKr0.chunk.mjs";import{b as J}from"./index-Vy5c5YOG.chunk.mjs";import{d as f,f as w,u as x,S as C,s as v,e as _}from"./files-CcMYUI1-.chunk.mjs";import{i as q}from"./index-ChxcQ8E-.chunk.mjs";import{p as D,x as O}from"./icons-DY4h9raQ.chunk.mjs";function R(t=[]){return`<?xml version="1.0"?>
			...
			</d:propfind>`}async function U(t,n,c=[],p=f){try{const r=await p.stat(t,{data:R(c),details:!0,...n});return h.debug("[Collections] Fetched a collection: ",{data:r.data}),B(r.data,t.split("/").slice(0,-1).join("/"))}catch(r){if(r instanceof DOMException&&r.code===r.ABORT_ERR)return null;throw r}}async function Y(t,n={},c=[],p=f){try{const r=await p.getDirectoryContents(t,{data:R(c),details:!0,...n});return h.debug(`[Collections] Fetched ${r.data.length} collections: `,{data:r.data}),r.data.filter(d=>d.filename!==t).map(d=>B(d,t))}catch(r){if(r instanceof DOMException&&r.code===r.ABORT_ERR)return[];throw r}}function B(t,n){t.props.collaborators===void 0||t.props.collaborators===""?t.props.collaborators=[]:typeof t.props.collaborators.collaborator=="object"&&(Array.isArray(t.props.collaborators.collaborator)?t.props.collaborators=t.props.collaborators.collaborator:t.props.collaborators=[t.props.collaborators.collaborator]);const c=JSON.parse(t.props.dateRange?.replace(/&quot;/g,'"')??"{}"),p=Math.floor(Date.now()/1e3),r=Number.isFinite(c.start)?c.start:p,d=Number.isFinite(c.end)?c.end:p,m={startDate:w(new Date(r*1e3)),endDate:w(new Date(d*1e3))};return m.startDate===m.endDate?t.props.date=m.startDate:t.props.date=s("photos","{startDate} to {endDate}",m),t.props.filters=JSON.parse(t.props.filters??"{}"),$(t,n)}async function K(t,n,c=[],p=f){try{const r=await p.getDirectoryContents(t,{data:M(c),details:!0,...n}),d=t.split("/").slice(0,-1).join("/"),m=r.data.map(b=>$(b,d,J("dav"))).filter(b=>b.fileid!==void 0);return h.debug(`[Collections] Fetched ${m.length} new files: `,{fetchedFiles:m}),m}catch(r){if(r instanceof DOMException&&r.code===r.ABORT_ERR)return[];throw h.error("Error fetching collection files",{error:r}),r}}const Q=["<nc:photos-collection-file-original-filename />"],X=P("collections",()=>{const t=D({}),n=D({});function c(e){return Object.values(t.value).filter(o=>o.root===e).reduce((o,a)=>({...o,[a.root+a.path]:a}),{})}function p(e){t.value={...t.value,...e.reduce((o,a)=>({...o,[a.root+a.path]:a}),{})}}function r(e){t.value[e.root+e.path]=e}function d(e){e.forEach(o=>{delete t.value[o],delete n.value[o]})}function m(e,o=[]){n.value={...n.value,[e]:o};const a=t.value[e];a!==void 0&&(a.attributes.nbItems=o.length,a.attributes["last-photo"]=Number.parseInt(o[o.length-1]))}function b(e,o){const a=n.value[e]||[];n.value={...n.value,[e]:[...new Set([...a,...o])]};const l=t.value[e];l.attributes.nbItems+=o.length,l.attributes["last-photo"]=Number.parseInt(o[o.length-1])}function N(e,o){n.value={...n.value,[e]:n.value[e].filter(l=>!o.includes(l))};const a=t.value[e];if(a.attributes.nbItems-=o.length,o.includes(a.attributes["last-photo"].toString())){const l=n.value[e];a.attributes["last-photo"]=Number.parseInt(l[l.length])}}async function j(e,o){const a=new C(5);b(e,o);const l=o.map(async u=>{const i=x().files[u],F=t.value[e],y=await a.acquire();try{await f.copyFile(i.root+i.path,`${F.root+F.path}/${i.basename}`)}catch(g){q(g)&&g.response?.status!==409&&(N(e,[u]),h.error(s("photos","Failed to add {fileBaseName} to collection {collectionFileName}",{fileBaseName:i.basename,collectionFileName:e}),{error:g}),v(s("photos","Failed to add {fileBaseName} to collection {collectionFileName}",{fileBaseName:i.basename,collectionFileName:e})))}finally{a.release(y)}});return Promise.all(l)}async function A(e,o){const a=new C(5);N(e,o);const l=o.map(async u=>{const i=x().files[u],F=await a.acquire();try{await f.deleteFile(i.root+i.path)}catch(y){b(e,[u]),h.error(s("photos","Failed to delete {fileBaseName}",{fileBaseName:i.basename}),{error:y}),v(s("photos","Failed to delete {fileBaseName}",{fileBaseName:i.basename}))}finally{a.release(F)}});return Promise.all(l)}async function E(e){try{return await f.createDirectory(e.root+e.path),p([e]),e}catch(o){h.error(s("photos","Failed to create {collectionFileName}",{collectionFileName:e.path}),{error:o}),v(s("photos","Failed to create {collectionFileName}",{collectionFileName:e.path}))}}async function I(e,o){const a=t.value[e],l=O(a).clone();l.rename(o);try{return p([l]),m(l.root+l.path,n.value[e]),await f.moveFile(a.root+a.path,a.root+l.path,{overwrite:!1}),d([e]),l}catch(u){return d([a.root+l.path]),h.error(s("photos","Failed to rename {currentCollectionFileName} to {newCollectionFileName}",{currentCollectionFileName:e,newCollectionFileName:l.path}),{error:u}),v(s("photos","Failed to rename {currentCollectionFileName} to {newCollectionFileName}",{currentCollectionFileName:e,newCollectionFileName:l.path})),a}}async function S(e,o){const a=t.value[e],l=O(a).clone();l.update(o);const u=Object.entries(o).map(([i,F])=>{switch(typeof F){case"string":return`<nc:${i}>${F}</nc:${i}>`;case"object":return`<nc:${i}>${JSON.stringify(F)}</nc:${i}>`;default:return""}}).join();try{return r(l),await f.customRequest(a.root+a.path,{method:"PROPPATCH",data:`<?xml version="1.0"?>

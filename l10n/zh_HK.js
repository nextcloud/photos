OC.L10N.register(
    "photos",
    {
    "Public link" : "公開連結",
    "Photos" : "照片",
    "On This Day" : "當年今日",
    "Your memories under your control" : "完全自主掌控您珍貴的回憶",
    "Search by file name" : "按檔案名稱搜尋",
    "Clear search" : "清除搜尋",
    "All media" : "所有媒體",
    "Videos" : "影片",
    "Albums" : "專輯 ",
    "Collaborative albums" : "協作相簿",
    "People" : "人物",
    "Folders" : "資料夾",
    "Favorites" : "我的最愛",
    "On this day" : "當年今日",
    "Memories" : "回憶",
    "Shared with you" : "與您分享",
    "Tags" : "標籤",
    "Places" : "地點",
    "Map" : "地圖",
    "Photos settings" : "照片設定",
    "Mark selection as favorite" : "將已選擇的標記為最愛",
    "Add selection to favorites" : "將選擇添加至最愛",
    "Remove selection from favorites" : "從最愛移除已選擇的範圍",
    "This name is already in use." : "這個用戶名稱已經有人使用了。",
    "\"{char}\" is not allowed inside a filename." : "檔案名稱中不允許出現「{char}」",
    "Invalid filename." : "檔案名稱無效。",
    "Name of the album" : "相簿名稱",
    "Location of the album" : "相簿位置",
    "Back" : "返回",
    "Add collaborators" : "添加協作者",
    "Save" : "保存",
    "Create album" : "創建相簿",
    "Add to Album" : "添加至相簿",
    "Add selection to album {albumName}" : "將選擇添加至相簿 {albumName}",
    "Shared by" : "分享自",
    "Create a new album." : "創建新相簿。",
    "Create new album" : "創建新相簿",
    "New album" : "新相簿",
    "_%n item_::_%n photos and videos_" : ["%n 個照片與視像"],
    "Failed to fetch collaborators list." : "擷取協作者清單失敗。",
    "Failed to update album." : "更新相簿失敗。",
    "Add people or groups who can edit your album" : "添加可以編輯您的相簿的用戶或群組",
    "Search people or groups" : "搜尋人或群組",
    "No recommendations. Start typing." : "沒有建議。開始輸入。",
    "Copy the public link" : "複製公開連結",
    "Public link copied!" : "公開連結已複製！",
    "Copy public link" : "複製公開連結",
    "Delete the public link" : "刪除公開連結",
    "Create public link share" : "建立公開連結分享",
    "Share via public link" : "透過公用連結分享",
    "This collection does not exist" : "此收藏品不存在",
    "An error occurred" : "發生錯誤",
    "No picture for this day" : "這天沒有照片",
    "Picture taken on this day will show up here." : "這天拍攝的照片將會顯示在此處。",
    "More photos from this day" : "更多這天的照片",
    "Jump to a month" : "跳至某個月份",
    "Memories illustration" : "回憶插圖",
    "Map illustration" : "地圖插圖",
    "Faces illustration" : "人臉插圖",
    "Photos illustration" : "相片插圖",
    "_%n photos_::_%n photos_" : ["%n 張照片"],
    "_%n unassigned photo_::_%n unassigned photos_" : ["%n 個未指派的照片"],
    "Favorite" : "我的最愛",
    "Favorite image, open the full size \"{name}\" image" : "收藏影像，開啟「{name}」的原始尺寸影像",
    "Open the full size \"{name}\" image" : "開啟「{name}」的原始尺寸影像",
    "{count} photos taken in one go" : "一次拍攝了 {count} 張相片",
    "Select image {imageName}" : "選擇圖像 {imageName}",
    "Open the \"{name}\" folder" : "開啟「{name}」資料夾",
    "Go back" : "返回",
    "Failed to add {fileName} to the favorites" : "無法將 {fileName} 加入最愛",
    "Failed to remove {fileName} from the favorites" : "無法從最愛移除 {fileName}",
    "Failed to delete {fileName}" : "刪除 {fileName} 失敗",
    "Actions for {name}" : "{name} 的操作",
    "View metadata" : "查看中繼資料",
    "Edit metadata" : "編輯元數據",
    "Remove from favorites" : "從最愛中移除",
    "Add to favorites" : "新增收藏",
    "Manage tags" : "管理標籤",
    "Add to album" : "添加至相簿",
    "Share" : "分享",
    "Delete" : "刪除",
    "Delete photo" : "刪除照片",
    "Are you sure you want to move \"{name}\" to the trash?" : "確定要將「{name}」移至垃圾桶嗎？",
    "Cancel" : "取消",
    "Move to trash" : "移至垃圾桶",
    "Photo metadata" : "相片中繼資料",
    "Filename" : "檔案名",
    "Place" : "地點",
    "Location" : "地點",
    "This photo carries no camera metadata." : "此相片沒有相機中繼資料。",
    "Both coordinates are required" : "必須提供兩個座標",
    "Enter decimal degrees between -{limit} and {limit}" : "輸入介乎 -{limit} 至 {limit} 之間的十進制度數",
    "Failed to save the metadata of {fileName}" : "無法儲存 {fileName} 的中繼資料",
    "Date and time the photo was taken" : "相片拍攝日期及時間",
    "Latitude" : "緯度",
    "Longitude" : "經度",
    "Remove location" : "移除位置",
    "{position} of {count}" : "第 {position} 張，共 {count} 張",
    "Failed to load the tags" : "無法載入標籤",
    "Failed to add the tag \"{tag}\"" : "無法新增標籤「{tag}」",
    "Failed to remove the tag \"{tag}\"" : "無法移除標籤「{tag}」",
    "A tag named \"{tag}\" already exists" : "名為「{tag}」的標籤已存在",
    "Failed to create the tag \"{tag}\"" : "無法建立標籤「{tag}」",
    "No tag exists yet, the field below creates the first one." : "尚未有任何標籤，可使用下方欄位建立第一個標籤。",
    "Tags of {name}" : "{name} 的標籤",
    "Create new tag" : "創建新標籤",
    "Add" : "添加",
    "Select a custom date range" : "選擇自訂日期範圍",
    "Select a date range" : "選取日期範圍",
    "Date range" : "日期範圍",
    "Select filters" : "選擇過濾器",
    "Dates" : "日期",
    "Jump to specific date in list" : "跳至清單中的特定日期",
    "Jump to {date}" : "跳到 {date}",
    "Add to {destination}" : "添加到 {destination}",
    "The destination folder is owned by {owner}" : "目標資料夾為 {owner} 擁有",
    "There are no photos or videos yet!" : "還沒有照片或視像！",
    "_{years} year ago_::_{years} years ago_" : ["{years} 年前"],
    "Squared photos view" : "方形照片檢視",
    "Slideshow" : "圖片播放",
    "Previous photo" : "上一張相片",
    "Next photo" : "下一張相片",
    "Play or pause the slideshow" : "播放或暫停投影片",
    "Close the slideshow" : "關閉投影片",
    "Show or hide photo metadata" : "顯示或隱藏相片中繼資料",
    "Delete source directory" : "刪除來源目錄",
    "Pick folder" : "挑選資料夾",
    "Select a source folder for your media" : "選擇您媒體的來源資料夾",
    "Media folders" : "媒體資料夾",
    "Choose the folders from where photos and videos are shown." : "選擇顯示照片與影片的資料夾。",
    "All folders" : "所有資料夾",
    "Add a Photos source for the timelines" : "添加時間軸的照片來源",
    "Add folder" : "添加資料夾",
    "Home" : "主頁",
    "Select the default upload location for your media" : "選擇媒體的默認上傳位置",
    "Upload folder" : "上傳資料夾",
    "General" : "一般",
    "_%n photo_::_%n photos_" : ["%n 張照片"],
    "Unselect all" : "取消全選",
    "Add photos to this album" : "添加照片至此相簿",
    "Manage collaborators for this album" : "管理此相簿的協作者",
    "Open actions menu" : "打開操作選項單",
    "Edit album details" : "編輯相簿的詳細資料",
    "Download all files in album" : "下載相簿中的所有檔案",
    "Delete album" : "刪除相簿",
    "Download selected files" : "下載已選擇的檔案",
    "Remove selection from album" : "從相簿移除已選擇的範圍",
    "This album does not have any photos or videos yet!" : "此相簿還沒有任何照片或視像！",
    "Add photos to {albumName}" : "添加照片至 {albumName}",
    "Manage collaborators" : "管理協作者",
    "Save collaborators for this album." : "保存此相簿的協作者。",
    "Cover photo for album {albumName}" : "{albumName} 相簿的封面照片。",
    "There is no album yet!" : "還沒有相簿！",
    "This person could not be found" : "找不到此人",
    "Rename person" : "重新命名人",
    "Merge with different person" : "與其他人合併",
    "Download selected photos" : "下載選定照片",
    "Remove person" : "移除人",
    "Name of this person" : "此人的名字",
    "Save." : "保存。",
    "Merge person" : "合併人",
    "Move to different person" : "移動至其他人",
    "_Move photo to a different person_::_Move photos to a different person_" : ["將照片移給其他人"],
    "_Remove photo from person_::_Remove photos from person_" : ["從人移除照片"],
    "This might take some time depending on the size of your photo library." : "這可能需要花一點時間，取決於您照片庫的大小。",
    "Recognized people will show up here" : "被識別的人將會出現在此處",
    "This folder does not exist" : "此資料夾不存在",
    "Loading folders…" : "載入資料夾…",
    "No photos in here" : "無照片檔案",
    "Open the Maps app" : "開啟地圖應用程式",
    "Looking for geotagged photos…" : "正在尋找已加入地理標記的相片…",
    "No geotagged photos" : "沒有已加入地理標記的相片",
    "Photos taken with a device recording its location are shown on this map." : "使用會記錄位置的裝置拍攝的相片會顯示在此地圖上。",
    "Play a slideshow of your {year} highlights" : "播放你的 {year} 年精彩回顧投影片",
    "Looking for memories…" : "正在尋找回憶…",
    "No memories yet" : "尚未有回憶",
    "Memories gather the photos of your trips and multi-day events." : "回憶會整理你的旅程及多日活動相片。",
    "Year in review" : "年度回顧",
    "Your {year} in photos" : "你的 {year} 年相片回顧",
    "_%n highlight_::_%n highlights_" : ["%n 精彩片段"],
    "_%n photo this year_::_%n photos this year_" : ["今年有 %n 張照片"],
    "Taken on {date} at {time}" : "攝於 {date} {time}",
    "This place does not have any photos or videos yet!" : "此處還沒有任何照片或影片！",
    "Add photos to this place" : "添加照片至此處",
    "Cover photo for place {placeName}" : "地點 {placeName} 的封面照片",
    "There is no place yet!" : "還沒有地點！",
    "Cover photo for shared album {albumName}" : "共享相簿 {albumName} 的封面照片",
    "Back to tags overview" : "返回標籤概覽",
    "No photos with this tag yet" : "還沒有帶有此標籤的照片",
    "No tags yet" : "尚未加入標籤",
    "Photos with tags will show up here" : "此處會顯示照片的標籤",
    "Popular tags" : "熱門標籤",
    "All tags" : "全部標籤",
    "Create new album from filters" : "從過濾器創建新相簿",
    "One of the source folders does not exist" : "其中一個來源資料夾不存在",
    "Change tile density" : "更改磚塊密度",
    "Density" : "密度",
    "Small tiles" : "小磚塊",
    "Default" : "默認",
    "Large tiles" : "大磚塊",
    "Start slideshow" : "開始放映幻燈片",
    "Delete selection" : "刪除所選",
    "No photos or videos in here" : "此處沒有照片或視像",
    "Unassigned faces" : "未指派的臉孔",
    "Error fetching files" : "擷取檔案時發生錯誤",
    "Failed to fetch collection." : "擷取收藏品失敗。",
    "Failed to fetch collections list." : "擷取收藏品清單失敗。",
    "Failed to fetch faces list." : "擷取臉部清單失敗。",
    "The folder {folder} does not exist anymore. You can remove it from your media folders in the Photos settings." : "資料夾 {folder} 已不存在。你可以在「相片」設定的媒體資料夾中將其移除。",
    "All your media" : "所有媒體",
    "Album {title}" : "相簿 {title}",
    "Shared Albums" : "已分享了的相簿",
    "Shared album {title}" : "已分享了的相簿 {title}",
    "Public album {title}" : "公共相簿 {title}",
    "Tagged photos" : "有標籤的照片",
    "Tagged photo {title}" : "有標籤的照片 {title}",
    "{title}'s face" : "{title} 的臉",
    "Last week" : "上星期",
    "Last Month" : "上個月",
    "Custom…" : "自訂 ...",
    "{startDate} to {endDate}" : "{startDate} 到 {endDate}",
    "Failed to add {fileBaseName} to collection {collectionFileName}" : "無法將 {fileBaseName} 添加到收藏 {collectionFileName}",
    "Failed to delete {fileBaseName}" : "刪除 {fileBaseName} 失敗",
    "Failed to create {collectionFileName}" : "無法創建 {collectionFileName}",
    "Failed to rename {currentCollectionFileName} to {newCollectionFileName}" : "重新命名 {currentCollectionFileName} 為 {newCollectionFileName} 失敗",
    "Failed to update properties of {collectionFileName} with {properties}" : "無法使用 {properties} 更新 {collectionFileName} 的屬性",
    "Delete collection" : "刪除收藏",
    "Leave shared album" : "離開已分享的相簿",
    "Are you sure you want to delete {collectionName}? This action cannot be undone." : "您確定您想要刪除 {collectionName} 嗎？此動作無法還原。",
    "Failed to delete {collectionFileName}" : "刪除 {collectionFileName} 失敗",
    "Failed to move {fileBaseName} to person {faceName}" : "無法將 {fileBaseName} 移動至人物 {faceName}",
    "Failed to remove {fileBaseName}" : "無法移除 {fileBaseName}",
    "Failed to rename {oldName} to {faceName}" : "重新命名 {oldName} 為 {faceName} 失敗",
    "Failed to delete {faceName}" : "刪除 {faceName} 失敗",
    "Failed to delete {fileId}" : "刪除 {fileId} 失敗",
    "Failed to set favorite state for {fileId}" : "未能為 {fileId} 設置收藏狀態",
    "Failed to set favorite state for {fileName}" : "未能為 {fileName} 設置收藏狀態",
    "Could not load photos folder" : "無法載入照片資料夾",
    "Camera" : "相機",
    "Aperture" : "光圈",
    "Focal length" : "焦距",
    "Exposure" : "曝光",
    "ISO" : "ISO",
    "{focalLength} mm" : "{focalLength} 毫米",
    "{exposureTime} s" : "{exposureTime} 秒",
    "Group" : "群組",
    "Remove {collaboratorLabel} from the collaborators list" : "從協作者清單中移除 {collaboratorLabel}",
    "Enable squared photos view" : "啟用方形照片顯示",
    "Choose default Photos upload and Albums location" : "選擇默認照片上傳及相簿位置",
    "Choose a different folder" : "選擇其他資料夾",
    "View" : "檢視",
    "Choose the folder where photos and albums are uploaded to." : "選擇照片與相簿上傳到的資料夾。",
    "Loading folders …" : "載入資料夾 …",
    "Details" : "細節",
    "Confirm" : "確認"
},
"nplurals=1; plural=0;");

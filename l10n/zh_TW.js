OC.L10N.register(
    "photos",
    {
    "Public link" : "公開連結",
    "Photos" : "照片",
    "On This Day" : "在這天",
    "Your memories under your control" : "完全自主掌控您珍貴的回憶",
    "Search by file name" : "按檔案名稱搜尋",
    "Clear search" : "清除搜尋",
    "All media" : "所有媒體",
    "Videos" : "影片",
    "Albums" : "相簿",
    "Collaborative albums" : "協作相簿",
    "People" : "人物",
    "Folders" : "資料夾",
    "Favorites" : "我的最愛",
    "On this day" : "在這天",
    "Memories" : "回憶",
    "Shared with you" : "與您分享",
    "Tags" : "標籤",
    "Places" : "地點",
    "Map" : "地圖",
    "Photos settings" : "照片設定",
    "Mark selection as favorite" : "將選取範圍標記為最愛",
    "Add selection to favorites" : "新增選取範圍至最愛",
    "Remove selection from favorites" : "從最愛移除選取範圍",
    "This name is already in use." : "此名稱已使用。",
    "\"{char}\" is not allowed inside a filename." : "檔案名稱中不允許出現「{char}」",
    "Invalid filename." : "無效的檔案名稱。",
    "Name of the album" : "相簿名稱",
    "Location of the album" : "相簿位置",
    "Back" : "返回",
    "Add collaborators" : "新增協作者",
    "Save" : "儲存",
    "Create album" : "建立相簿",
    "Add to Album" : "新增至相簿",
    "Add selection to album {albumName}" : "新增選取範圍至相簿 {albumName}",
    "Shared by" : "分享自",
    "Create a new album." : "建立新相簿。",
    "Create new album" : "建立新相簿",
    "New album" : "新相簿",
    "_%n item_::_%n photos and videos_" : ["%n 個照片與影片"],
    "Failed to fetch collaborators list." : "擷取協作者清單失敗。",
    "Failed to update album." : "更新相簿失敗。",
    "Add people or groups who can edit your album" : "新增可以編輯您的相簿的使用者或群組",
    "Search people or groups" : "搜尋使用者或群組",
    "No recommendations. Start typing." : "無建議。開始輸入。",
    "Copy the public link" : "複製公開連結",
    "Public link copied!" : "已複製公開連結！",
    "Copy public link" : "複製公開連結",
    "Delete the public link" : "刪除公開連結",
    "Create public link share" : "建立公開連結分享",
    "Share via public link" : "透過公開連結分享",
    "This collection does not exist" : "此收藏不存在",
    "An error occurred" : "發生錯誤",
    "No picture for this day" : "這天沒有照片",
    "Picture taken on this day will show up here." : "這天拍攝的照片將會顯示在此處。",
    "More photos from this day" : "更多這天的照片",
    "Jump to a month" : "跳至一個月",
    "Memories illustration" : "回憶插圖",
    "Map illustration" : "地圖插圖",
    "Faces illustration" : "臉部插圖",
    "Photos illustration" : "照片插圖",
    "_%n photos_::_%n photos_" : ["%n 個照片"],
    "_%n unassigned photo_::_%n unassigned photos_" : ["%n 個未指派的照片"],
    "Favorite" : "收藏",
    "Favorite image, open the full size \"{name}\" image" : "收藏影像，開啟「{name}」的原始尺寸影像",
    "Open the full size \"{name}\" image" : "開啟「{name}」的原始尺寸影像",
    "{count} photos taken in one go" : "一次拍攝的 {count} 張照片",
    "Select image {imageName}" : "選取圖片 {imageName}",
    "Open the \"{name}\" folder" : "開啟「{name}」資料夾",
    "Go back" : "返回",
    "Failed to add {fileName} to the favorites" : "新增 {fileName} 到收藏失敗",
    "Failed to remove {fileName} from the favorites" : "從收藏移除 {fileName} 失敗",
    "Failed to delete {fileName}" : "刪除 {fileName} 失敗",
    "Actions for {name}" : "{name} 的動作",
    "View metadata" : "檢視中介資料",
    "Edit metadata" : "編輯詮釋資料",
    "Remove from favorites" : "從最愛中移除",
    "Add to favorites" : "新增至喜愛",
    "Manage tags" : "管理標籤",
    "Add to album" : "新增至相簿",
    "Share" : "分享",
    "Delete" : "刪除",
    "Delete photo" : "刪除照片",
    "Are you sure you want to move \"{name}\" to the trash?" : "您真的想要移動「{name}」到回收桶嗎？",
    "Cancel" : "取消",
    "Move to trash" : "移動到回收桶",
    "Photo metadata" : "照片中介資料",
    "Filename" : "檔案名稱",
    "Place" : "地點",
    "Location" : "位置",
    "This photo carries no camera metadata." : "此照片沒有相機的中介資料",
    "Both coordinates are required" : "兩組座標皆為必填項目",
    "Enter decimal degrees between -{limit} and {limit}" : "請輸入介於˙-{limit} 到 {limit} 間的十進位度數",
    "Failed to save the metadata of {fileName}" : "無法儲存 {fileName} 的中介資料",
    "Date and time the photo was taken" : "照片拍攝的日期與時間",
    "Latitude" : "緯度",
    "Longitude" : "經度",
    "Remove location" : "移除位置",
    "{position} of {count}" : "第 {count} 項的 {position}",
    "Failed to load the tags" : "載入標籤失敗",
    "Failed to add the tag \"{tag}\"" : "新增標籤「{tag}」失敗",
    "Failed to remove the tag \"{tag}\"" : "移除標籤「{tag}」失敗",
    "A tag named \"{tag}\" already exists" : "已有名為「{tag}」的標籤",
    "Failed to create the tag \"{tag}\"" : "建立標籤「{tag}」失敗",
    "No tag exists yet, the field below creates the first one." : "尚無標籤，填寫下方欄位即可建立第一個標籤。",
    "Tags of {name}" : "{name} 的標籤",
    "Create new tag" : "建立新標籤",
    "Add" : "加",
    "Select a custom date range" : "選取自訂日期範圍",
    "Select a date range" : "選取日期範圍",
    "Date range" : "日期範圍",
    "Select filters" : "選取篩選條件",
    "Dates" : "日曆",
    "Jump to specific date in list" : "跳到清單中的指定日期",
    "Jump to {date}" : "跳到 {date}",
    "Add to {destination}" : "新增至 {destination}",
    "The destination folder is owned by {owner}" : "目標資料夾為 {owner} 擁有",
    "There are no photos or videos yet!" : "還沒有照片或影片！",
    "_{years} year ago_::_{years} years ago_" : ["{years} 年前"],
    "Squared photos view" : "方形照片檢視",
    "Slideshow" : "幻燈片",
    "Previous photo" : "前一張照片",
    "Next photo" : "下一張照片",
    "Play or pause the slideshow" : "播放或暫停投影片",
    "Close the slideshow" : "關閉投影片",
    "Show or hide photo metadata" : "顯示或隱藏照片中介資料",
    "Delete source directory" : "刪除來源目錄",
    "Pick folder" : "挑選資料夾",
    "Select a source folder for your media" : "選取您媒體的來源資料夾",
    "Media folders" : "媒體資料夾",
    "Choose the folders from where photos and videos are shown." : "選擇顯示照片與影片的資料夾。",
    "All folders" : "所有資料夾",
    "Add a Photos source for the timelines" : "新增時間軸的照片來源",
    "Add folder" : "新增資料夾",
    "Home" : "家庭",
    "Select the default upload location for your media" : "選取您媒體的預設上傳位置",
    "Upload folder" : "上傳資料夾",
    "General" : "一般",
    "_%n photo_::_%n photos_" : ["%n 張照片"],
    "Unselect all" : "取消全選",
    "Add photos to this album" : "新增照片至此相簿",
    "Manage collaborators for this album" : "管理此相簿的協作者",
    "Open actions menu" : "開啟動作選單",
    "Edit album details" : "編輯相簿詳細資訊",
    "Download all files in album" : "下載相簿中的所有檔案",
    "Delete album" : "刪除相簿",
    "Download selected files" : "下載已選取的檔案",
    "Remove selection from album" : "從相簿移除選取範圍",
    "This album does not have any photos or videos yet!" : "此相簿還沒有任何照片或影片！",
    "Add photos to {albumName}" : "新增照片至 {albumName}",
    "Manage collaborators" : "管理協作者",
    "Save collaborators for this album." : "儲存此相簿的協作者。",
    "Cover photo for album {albumName}" : "相簿 {albumName} 的封面照片",
    "There is no album yet!" : "尚無相簿！",
    "This person could not be found" : "找不到此人",
    "Rename person" : "重新命名人",
    "Merge with different person" : "與其他人合併",
    "Download selected photos" : "下載選定的照片",
    "Remove person" : "移除人",
    "Name of this person" : "此人的名字",
    "Save." : "儲存。",
    "Merge person" : "合併人",
    "Move to different person" : "移動至其他人",
    "_Move photo to a different person_::_Move photos to a different person_" : ["將照片移動為其他人"],
    "_Remove photo from person_::_Remove photos from person_" : ["從人移除照片"],
    "This might take some time depending on the size of your photo library." : "這可能需要花一點時間，取決於您照片庫的大小。",
    "Recognized people will show up here" : "被識別的人將會出現在此處",
    "This folder does not exist" : "此資料夾不存在",
    "Loading folders…" : "載入資料夾…",
    "No photos in here" : "此處無圖片檔案",
    "Open the Maps app" : "開啟「地圖」應用程式",
    "Looking for geotagged photos…" : "正在尋找有地理標籤的照片⋯⋯",
    "No geotagged photos" : "沒有有地理標籤的照片",
    "Photos taken with a device recording its location are shown on this map." : "這張地圖上顯示的是使用具備定位功能的裝置所拍攝的照片。",
    "Play a slideshow of your {year} highlights" : "播放 {year} 年度精彩片段的幻燈片",
    "Looking for memories…" : "正在尋找回憶⋯⋯",
    "No memories yet" : "尚無回憶",
    "Memories gather the photos of your trips and multi-day events." : "「回憶」專區彙整了您的旅行照片及為期數天的活動照片。",
    "Year in review" : "年度回顧",
    "Your {year} in photos" : "您的 {year} 照片回顧",
    "_%n highlight_::_%n highlights_" : ["%n 精彩片段"],
    "_%n photo this year_::_%n photos this year_" : ["今年有 %n 張照片"],
    "Taken on {date} at {time}" : "拍攝於 {date} 的 {time}",
    "This place does not have any photos or videos yet!" : "此處還沒有任何照片或影片！",
    "Add photos to this place" : "新增照片至此處",
    "Cover photo for place {placeName}" : "地點 {placeName} 的封面照片",
    "There is no place yet!" : "還沒有地點！",
    "Cover photo for shared album {albumName}" : "共享相簿 {albumName} 的封面照片",
    "Back to tags overview" : "回到標籤概覽",
    "No photos with this tag yet" : "還沒有帶有此標籤的照片",
    "No tags yet" : "尚未加入標籤",
    "Photos with tags will show up here" : "此處會顯示圖片的標籤",
    "Popular tags" : "熱門標籤",
    "All tags" : "全部標籤",
    "Create new album from filters" : "從篩選條件建立新相簿",
    "One of the source folders does not exist" : "其中一個來源資料夾不存在",
    "Change tile density" : "變更圖磚密度",
    "Density" : "密度",
    "Small tiles" : "小圖磚",
    "Default" : "預設",
    "Large tiles" : "大圖磚",
    "Start slideshow" : "開始幻燈片",
    "Delete selection" : "刪除選擇",
    "No photos or videos in here" : "此處沒有照片或影片",
    "Unassigned faces" : "未指派的臉孔",
    "Error fetching files" : "擷取檔案時發生錯誤",
    "Failed to fetch collection." : "擷取收藏品失敗。",
    "Failed to fetch collections list." : "擷取收藏清單失敗。",
    "Failed to fetch faces list." : "擷取臉部清單失敗。",
    "All your media" : "您所有的媒體",
    "Album {title}" : "相簿 {title}",
    "Shared Albums" : "已分享的相簿",
    "Shared album {title}" : "已分享的相簿 {title}",
    "Public album {title}" : "公開相簿 {title}",
    "Tagged photos" : "有標籤的照片",
    "Tagged photo {title}" : "已標記的照片 {title}",
    "{title}'s face" : "{title} 的臉",
    "Last week" : "上星期",
    "Last Month" : "上個月",
    "Custom…" : "自訂……",
    "{startDate} to {endDate}" : "{startDate} 到 {endDate}",
    "Failed to add {fileBaseName} to collection {collectionFileName}" : "無法將 {fileBaseName} 新增至收藏 {collectionFileName}",
    "Failed to delete {fileBaseName}" : "刪除 {fileBaseName} 失敗",
    "Failed to create {collectionFileName}" : "建立 {collectionFileName} 失敗",
    "Failed to rename {currentCollectionFileName} to {newCollectionFileName}" : "無法將 {currentCollectionFileName} 重新命名為 {newCollectionFileName}",
    "Failed to update properties of {collectionFileName} with {properties}" : "無法使用 {properties} 更新 {collectionFileName} 的屬性",
    "Delete collection" : "刪除收藏",
    "Leave shared album" : "離開已分享的相簿",
    "Are you sure you want to delete {collectionName}? This action cannot be undone." : "您確定您想要刪除 {collectionName} 嗎？此動作無法還原。",
    "Failed to delete {collectionFileName}" : "刪除 {collectionFileName} 失敗",
    "Failed to move {fileBaseName} to person {faceName}" : "將 {fileBaseName} 移動到人物 {faceName} 失敗",
    "Failed to remove {fileBaseName}" : "移除 {fileBaseName} 失敗",
    "Failed to rename {oldName} to {faceName}" : "無法將 {oldName} 重新命名為 {faceName}",
    "Failed to delete {faceName}" : "刪除 {faceName} 失敗",
    "Failed to delete {fileId}" : "刪除 {fileId} 失敗",
    "Failed to set favorite state for {fileId}" : "無法設定 {fileId} 的收藏狀態",
    "Failed to set favorite state for {fileName}" : "無法設定 {fileName} 的收藏狀態",
    "Could not load photos folder" : "無法載入照片資料夾",
    "Camera" : "攝影機",
    "Aperture" : "光圈",
    "Focal length" : "焦距",
    "Exposure" : "曝光",
    "ISO" : "ISO",
    "{focalLength} mm" : "{focalLength} 公釐",
    "{exposureTime} s" : "{exposureTime} 秒",
    "Group" : "群組",
    "Remove {collaboratorLabel} from the collaborators list" : "從協作者清單移除 {collaboratorLabel}",
    "Enable squared photos view" : "啟用方形照片檢視",
    "Choose default Photos upload and Albums location" : "選擇預設照片上傳與相簿位置",
    "Choose a different folder" : "選擇其他資料夾",
    "View" : "檢視",
    "Choose the folder where photos and albums are uploaded to." : "選擇照片與相簿上傳到的資料夾。",
    "Loading folders …" : "正在載入資料夾……",
    "Details" : "詳細資訊",
    "Confirm" : "確認"
},
"nplurals=1; plural=0;");

...
export async function assignTagToFile(fileId: number, tag: PhotoTag): Promise<void> {
	...
	try {
		// An object body is what makes the DAV client send `application/json`,
		// the only content type the tag endpoints accept.
...
/**
 * Whether a rejected {@link createTag} was refused because the account already
 * has a tag of that name.
 *
 * @param error - The error `createTag` threw
 */
export function isTagNameTaken(error: unknown): boolean {
	return (error as WebDAVClientError)?.response?.status === 409
}
...
export async function createTag(displayName: string): Promise<number> {
	const response = await davClient.customRequest('/systemtags', {
		method: 'POST',
		// An object body, see `assignTagToFile`.
		data: {
			name: displayName,
			userVisible: true,
			userAssignable: true,
